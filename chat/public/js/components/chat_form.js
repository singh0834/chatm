...
export default class ChatForm {
  ...
  setup_header() {
    this.avatar_html = frappe.avatar(null, 'avatar-medium', 'OneHash Chat');
    const header_html = `
  ...
  setup_form() {
    const form_html = `
		...
		`;
    const footer_html = `
      <a class='chat-footer' target='_blank' href='https://www.onehash.ai/'>
        ${__('⚡ Powered by OneHash')}
      </a>
    `;
